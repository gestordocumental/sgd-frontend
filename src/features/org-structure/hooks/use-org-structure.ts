...
export function useOrgStructure(companyId: string, enabled = true) {
  ...
  // ── Forms ──────────────────────────────────────────────────────
  const deptForm = useForm<StructureForm>({
    resolver: zodResolver(structureSchema),
    mode: 'onChange',
  });
  const areaForm = useForm<StructureForm>({
    resolver: zodResolver(structureSchema),
    mode: 'onChange',
  });
  const cargoForm = useForm<StructureForm>({
    resolver: zodResolver(structureSchema),
    mode: 'onChange',
  });
  const deptCargoForm = useForm<StructureForm>({
    resolver: zodResolver(structureSchema),
    mode: 'onChange',
  });
  ...
  const invalidateAreas = () => queryClient.invalidateQueries({ queryKey: ['areas', companyId] });
  ...
  const createAreaMutation = useMutation({
    mutationFn: (dto: StructureForm) => orgStructureApi.createArea(companyId, selectedDeptId, dto),
    onSuccess: () => {
      invalidateAreas();
  ...
  const editDeptCargoMutation = useMutation({
    mutationFn: (dto: StructureForm) =>
      orgStructureApi.updateDeptCargo(
        companyId,
        editDeptCargo!.departamentoId,
        editDeptCargo!.id,
        dto,
      ),
    onSuccess: () => {
  ...
  const deleteDeptCargoMutation = useMutation({
    mutationFn: (c: ApiCargo) => orgStructureApi.deleteDeptCargo(companyId, c.departamentoId, c.id),
    onSuccess: () => {
      invalidateCargos();
