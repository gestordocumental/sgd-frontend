...
import { Plus, Pencil, Trash2, History, Eye, Download, FileUp, RefreshCw, Copy } from 'lucide-react'
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <SearchInput value={typoSearch} onChange={(v) => { setTypoSearch(v); setTypoPage(1) }} placeholder={t('common.search')} />
            <select
              value={typoStatus}
              ...
              <TableBody>
                {paginatedTypos.map((typo: ApiTypology) => (
                  <TableRow key={typo.id}>
                    ...
                    <TableCell className="text-sm">{typo.estructuraOrg.departamentoNombre}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {[typo.estructuraOrg.areaNombre, typo.estructuraOrg.cargoNombre].filter(Boolean).join(' / ') || '—'}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-xs">
                      <span className="flex items-center gap-1 min-w-0">
                        ...
                        <Button variant="ghost" size="sm" className="h-5 w-5 p-0 shrink-0" title={t('audit.detail.copy')} onClick={() => navigator.clipboard.writeText(typo.id)}>
