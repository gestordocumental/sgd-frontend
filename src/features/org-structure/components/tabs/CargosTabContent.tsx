...
export function CargosTabContent({ hook, canWrite = false }: CargosTabContentProps) {
  ...
  const {
    departamentos,
    areas, cargos, cargosLoading, deptCargos, deptCargosLoading,
  ...
  const filteredCargos = activeCargos.filter((c) => !cargoSearch || c.name.toLowerCase().includes(cargoSearch.toLowerCase()))
  ...
  return (
    <TabsContent value="cargos" className="mt-4 space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {selectedDeptId && (
          <SearchInput value={cargoSearch} onChange={(v) => { setCargoSearch(v); setCargoPage(1) }} placeholder={t('common.search')} />
        )}
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        {!selectedDeptId ? (
          ...
          <>
            ...
            {cargoTotalPages > 1 && (
              <Pager page={cargoSafePage} totalPages={cargoTotalPages} total={filteredCargos.length} onChange={cargoPage => setCargoPage(cargoPage)} />
            )}
          </>
        )}
      </div>
