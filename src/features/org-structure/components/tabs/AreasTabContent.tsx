...
export function AreasTabContent({ hook, canWrite = false }: AreasTabContentProps) {
  ...
  const filteredAreas  = areas.filter((a) => !areaSearch || a.name.toLowerCase().includes(areaSearch.toLowerCase()))
  ...
  return (
    <TabsContent value="areas" className="mt-4 space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {selectedDeptId && (
          <SearchInput value={areaSearch} onChange={(v) => { setAreaSearch(v); setAreaPage(1) }} placeholder={t('common.search')} />
        )}
