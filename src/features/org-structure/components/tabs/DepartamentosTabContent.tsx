...
function downloadTemplate() {
  const headers = [
    'Departamento', 'Descripción Departamento', 'Área',
    'Descripción Área', 'Cargo', 'Descripción Cargo',
  ]
  const colWidths = [
...
export function DepartamentosTabContent({ hook, canWrite = false }: DepartamentosTabContentProps) {
  ...
  const { departamentos, deptLoading, setCreateDeptOpen, openEditDept, setDeleteDept, bulkImportMutation } = hook
  ...
  const filteredDepts  = departamentos.filter((d) => !deptSearch || d.name.toLowerCase().includes(deptSearch.toLowerCase()))
        ...
        <div className="rounded-lg border border-border bg-card overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <SearchInput value={deptSearch} onChange={(v) => { setDeptSearch(v); setDeptPage(1) }} placeholder={t('common.search')} />
              {canWrite && (
                <>
