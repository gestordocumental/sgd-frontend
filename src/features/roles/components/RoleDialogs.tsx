  ...
  return (
    <>
      ...
      <Dialog open={createRoleOpen} onOpenChange={setCreateRoleOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          >
            <FormField
              id="cr-name"
              label={t('roles.dialogs.roleNameLabel')}
              error={createForm.formState.errors.name?.message}
            >
              <Input
                id="cr-name"
                placeholder={t('roles.dialogs.roleNamePlaceholder')}
            ...
            >
              <Input
                id="cr-desc"
                placeholder={t('roles.dialogs.descriptionPlaceholder')}
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={createRoleMutation.isPending || !createForm.formState.isValid}
              >
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{t('roles.dialogs.editTitle')}</DialogTitle>
          </DialogHeader>
          <form
          ...
          >
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={editRoleMutation.isPending || !editForm.formState.isValid}
              >
      ...
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {t('roles.dialogs.assignRoleTitle', { roleName: assignRoleUser?.role.name })}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-1 py-2">
            ...
            {activeUsers.filter((u) => !u.roles.some((r) => r.roleId === assignRoleUser?.role.id))
              .length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-4">
                {t('roles.dialogs.allUsersHaveRole')}
              </p>
            )}
          </div>
          <DialogFooter>
