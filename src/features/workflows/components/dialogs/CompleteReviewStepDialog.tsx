...
export function CompleteReviewStepDialog({ hook }: { hook: WorkflowsHook }) {
  ...
  return (
    <Dialog
      open={!!completeStepWorkflow}
      onOpenChange={(o) => {
        if (!o) setCompleteStepWorkflow(null);
      }}
    >
      <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex flex-col gap-4 pt-1 min-h-0 flex-1 overflow-y-auto pr-1">
          <p className="text-sm text-muted-foreground shrink-0">
            ...
            {currentStep ? (
              <span className="font-medium text-foreground">#{currentStep.stepOrder}</span>
            ) : null}{' '}
        ...
        <DialogFooter className="pt-2 shrink-0">
          <Button type="button" variant="outline" onClick={() => setCompleteStepWorkflow(null)}>
            {t('common.cancel')}
          </Button>
          <Button
            type="button"
