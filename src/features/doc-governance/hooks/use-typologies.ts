...
export function useTypologies(orgId: string, enabled = true) {
  ...
  const { data: formCargos = [] } = useQuery({
  ...
  const editMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); setEditTypology(null) },
    onError: (e: { response?: { data?: { message?: string } } }) => {
      const msg = e.response?.data?.message
      if (msg) form.setError('root', { message: msg })
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); setDeleteTypology(null) },
  })
  ...
  const uploadDocMutation = useMutation({
    mutationFn: async ({ values, file }: { values: UploadDocForm; file: File }) => {
      ...
      if (values.nombre  && values.nombre  !== (typo.datosDeclarados.nombre  ?? '')) patchDto.nombre  = values.nombre
      if (values.version && values.version !== (typo.datosDeclarados.version ?? '')) patchDto.version = values.version
  ...
  return {
    // Data
    typologies, isLoading,

    // Form org-structure data
    departamentos, formAreas, formCargos,
    formDeptId, formAreaId,
    handleFormDeptChange, handleFormAreaChange,

    // Create/Edit form & state
    form,
    createOpen,    setCreateOpen,    openCreate,
    editTypology,  setEditTypology,  openEdit,
    deleteTypology, setDeleteTypology,

    // File for create dialog
    createFile, setCreateFile,

    // Upload document dialog
    uploadDocTypology, setUploadDocTypology, openUploadDoc,
