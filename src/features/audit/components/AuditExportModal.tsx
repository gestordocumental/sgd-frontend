    ...
    try {
      ...
      const rows = data.map((log) => {
        ...
        return {
          ...
          [t('audit.columns.actor')]:        log.actorId,
        ...
        <div className="space-y-4 text-sm">
          ...
          {!byCorrelation && (
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-muted-foreground">{t('audit.export.maxRecords')}</label>
                <select
                ...
                >
                  {MAX_LIMIT_OPTIONS.map((n) => (
                    <option key={n} value={n}>{n.toLocaleString()} {t('audit.events')}</option>
                  ))}
                </select>
              </div>
            </div>
