...
import { LogOut, ChevronsUpDown, Building2, ShieldCheck, Check, MapPin, Phone, Hash, Briefcase, CreditCard, User } from 'lucide-react'
...
export function UserProfileCard() {
  ...
  const contextLabel = currentCompanyId
    ? (companies.find((c) => c.id === currentCompanyId)?.name ?? user?.companyName ?? currentCompanyId)
          ...
          <DropdownMenuGroup>
            ...
            <div className="px-1.5 pb-1 space-y-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium truncate flex-1">{fullName ?? t('common.user')}</span>
              ...
              <>
                ...
                {companies.map((company) => (
                  ...
                  >
                    ...
                    <span className="flex-1 text-xs truncate">{company.name}</span>
                    {currentCompanyId === company.id && <Check className="size-3.5 text-primary" />}
                  </DropdownMenuItem>
                ))}
              </>
            )}
              ...
              <DropdownMenuGroup>
                ...
                <div className="px-1.5 py-1 space-y-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`shrink-0 text-[10px] px-1 rounded-sm font-medium ${currentCompany.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-muted text-muted-foreground'}`}>
                  ...
                  {currentCompany.address && (
                    <div className="flex items-start gap-1.5">
                      ...
                      <span className="text-xs text-muted-foreground leading-tight">{currentCompany.address}</span>
                    </div>
                  )}
