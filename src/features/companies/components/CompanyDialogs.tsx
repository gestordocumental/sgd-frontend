      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={createForm.handleSubmit(onCreateSubmit)} className="space-y-4 pt-2">
            <FormField
              id="company-name"
              label={t('companies.dialogs.companyNameLabel')}
              error={createForm.formState.errors.name?.message}
            >
              <Input
                id="company-name"
                placeholder={t('companies.dialogs.companyNamePlaceholder')}
                {...createForm.register('name')}
              />
            </FormField>
            <FormField
              id="company-nit"
              label={t('companies.nit')}
              error={createForm.formState.errors.nit?.message}
            >
              <Input
                id="company-nit"
                placeholder={t('companies.dialogs.nitPlaceholder')}
            ...
            >
              <Input
                id="company-address"
                placeholder={t('companies.dialogs.addressPlaceholder')}
            ...
            >
              <Input
                id="company-phone"
                placeholder={t('companies.dialogs.phonePlaceholder')}
            ...
            <DialogFooter className="pt-2">
              ...
              >
                {createMutation.isPending
                  ? t('common.creating')
                  : t('companies.dialogs.createButton')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
          ...
          <DialogFooter className="pt-2">
            ...
            >
              {deleteMutation.isPending
                ? t('common.deleting')
                : t('companies.dialogs.deleteButton')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
