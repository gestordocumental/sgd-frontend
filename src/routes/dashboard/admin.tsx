...
import {
  ...
  Building2, ChevronRight, ChevronDown, CheckCircle, XCircle,
} from 'lucide-react'
...
function AdminDashboard() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { invalidateUsers(); setCreateOpen(false) },
  })

  const editMutation = useMutation({
    mutationFn: ({ id, dto }: { id: string; dto: UpdateUserDto }) => usersApi.update(id, dto),
    onSuccess: () => { invalidateUsers(); setEditUser(null) },
  })

  const deleteMutation = useMutation({
    mutationFn: (id: string) => usersApi.remove(id),
    onSuccess: () => { invalidateUsers(); setDeleteUser(null) },
  })
  ...
  const createCompanyMutation = useMutation({
    ...
    onSuccess: () => { invalidateCompanies(); setCreateCompanyOpen(false) },
  })
  ...
  const onCompanyCreateSubmit = (values: CompanyForm) => createCompanyMutation.mutate(values)
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      >
        ...
        <header className="flex items-center justify-between px-6 h-16 border-b border-border bg-card shrink-0">
          ...
          {activeTab === 'users' ? (
            ...
            <Button size="sm" onClick={() => { companyCreateForm.reset(); setCreateCompanyOpen(true) }}>
              <Building2 className="size-4" />
              Nueva empresa
            </Button>
          )}
        ...
        <TabsContent value="companies" className="overflow-auto">
          <main className="p-6 space-y-6">
            ...
            <div className={`grid grid-cols-1 gap-4 ${selectedCompany ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
              <StatCard
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onClick={() => toggleCompanyStatusMutation.mutate(company.id)}
                                >
                                  {company.status === 'active' ? (
                                    <><XCircle className="size-4" />Desactivar empresa</>
                                  ) : (
                                    <><CheckCircle className="size-4" />Activar empresa</>
                                  )}
                                </DropdownMenuItem>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem
      ...
      <Dialog open={createCompanyOpen} onOpenChange={setCreateCompanyOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={companyCreateForm.handleSubmit(onCompanyCreateSubmit)} className="space-y-4 pt-2">
            <FormField
              id="company-name"
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={createCompanyMutation.isPending || !companyCreateForm.formState.isValid}
              >
      ...
      <Dialog open={!!editCompany} onOpenChange={(open) => { if (!open) setEditCompany(null) }}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={companyEditForm.handleSubmit(onCompanyEditSubmit)} className="space-y-4 pt-2">
            <FormField
            ...
            <DialogFooter className="pt-2">
              ...
              <Button
                type="submit"
                disabled={editCompanyMutation.isPending || !companyEditForm.formState.isValid}
              >
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      ...
                      <TableHead className="text-xs py-2.5">Último acceso</TableHead>
                  ...
                  <TableBody>
                    {users.map((u) => (
                      <TableRow key={u.id} className={isDeleted(u) ? 'opacity-50' : ''}>
                        ...
                        <TableCell className="py-2.5 text-sm text-muted-foreground">—</TableCell>
                        <TableCell className="py-2.5">
                        ...
                        <TableCell className="py-2.5">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem>
                                {isDeleted(u) ? (
                                  <><CheckCircle className="size-4" />Activar usuario</>
                                ) : (
                                  <><XCircle className="size-4" />Desactivar usuario</>
                                )}
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
