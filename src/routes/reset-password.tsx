  ...
  const { mutate: resetPwd, isPending } = useMutation({
    ...
    onError: (error: AxiosError<{ message: string | string[] }>) => {
      ...
      const msg = Array.isArray(raw)
        ? raw[0]
        : (raw ?? t('auth.resetPasswordPage.serverErrorFallback'));
      setServerError(msg);
    },
  });
        ...
        <div className="w-full max-w-sm">
            ...
            <div className="text-center space-y-5">
              ...
              <div>
                <h2 className="text-xl font-semibold">
                  {t('auth.resetPasswordPage.invalidToken.title')}
                </h2>
                <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">
                  {t('auth.resetPasswordPage.invalidToken.description')}
                </p>
              </div>
              <Button variant="outline" className="w-full" onClick={goToLogin}>
                {t('auth.resetPasswordPage.invalidToken.goToSignIn')}
              </Button>
            </div>
          )}
          ...
          {token && success && (
            <div className="text-center space-y-5">
              ...
              <Button className="w-full" size="lg" onClick={goToLogin}>
                {t('auth.resetPasswordPage.success.goToSignIn')}
              </Button>
            </div>
          )}
