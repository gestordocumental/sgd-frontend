...
export const rolesApi = {
  ...
  deleteRole: (id: string) => apiClient.delete<void>(`/roles/${id}`).then((r) => r.data),

  // Replaces all permissions on a role
  assignPermissions: (roleId: string, permissionIds: string[]) =>
    apiClient.post<ApiRole>(`/roles/${roleId}/permissions`, { permissionIds }).then((r) => r.data),

  removePermission: (roleId: string, permissionId: string) =>
    apiClient.delete<ApiRole>(`/roles/${roleId}/permissions/${permissionId}`).then((r) => r.data),
};
